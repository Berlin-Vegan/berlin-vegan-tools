function updateMarkers(markersArray){
    infowindow.close();
    var districtElem = document.getElementById("districtselect");
    selectedDistrict = "";
    if(districtElem.selectedIndex !=0) {
        selectedDistrict = districtElem.options[districtElem.selectedIndex].value;
    }
    for( var i=0; i<markersArray.length; i++ ){
        marker = markersArray[i];
        showMarker=true;
        if(selectedDistrict != "" && selectedDistrict != marker.bv_district) {
            showMarker = false;
        }
